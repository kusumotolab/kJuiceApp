...
public class MembersControllerTest {
  ...
  public void normalGetAllMembers() throws Exception {
    var expectedResponseParams =
        List.of(new MemberResponseBody("h-takesg", "竹重", "m1", true));
  ...
  void normalUpdateMember() throws Exception {
    ...
    var requestParams = new MemberUpdateRequestBody(Optional.empty(), Optional.of("m2"), Optional.of(false));
    String requestBody = objectMapper.writeValueAsString(requestParams);
    ...
    this.mockMvc
      .perform(patch("/members/" + targetId).contentType(MediaType.APPLICATION_JSON).content(requestBody))
      .andDo(print())
      .andExpect(status().isOk())
      .andExpect(content().json(expectedResponseBody, true));
  }
